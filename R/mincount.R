...
### calculate the coefficients of the power series with freq >= k given the
...
preseqR.interpolate.mincount <- function(ss, hist, k=1)
{
  ...
  expect.distinct <- function(hist, N, n, S, k) {
    denom <- lchoose(N, n)
    p <- sapply(hist[, 1], function(x) {
               sum(exp(lchoose(N - x, n - 0:(k-1)) + lchoose(x, 0:(k-1)) - denom))
    })
    return(S - p %*% hist[, 2])
  }
  ...
  yield.estimates <- sapply(x, function(x) expect.distinct(hist, N, x, initial.distinct, k))
     ...
     step.size = NULL, max.size = NULL, k)
{
  ...
  if (length(hist[, 2]) >= k) {
    initial_sum <- floor(sum(hist[k:length(hist[, 2]), 2]))
  } else {
    initial_sum <- 0
  }
...
##  }
  ...
  res <- preseqR.extrapolate.mincount(hist, CF, start, step, end, k)
